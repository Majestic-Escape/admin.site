...
const BOOKING_FILTERS = { q: "", status: "all", from: "", to: "" };
...
export default function BookingsPage() {
  ...
  // page / size / sort / search / status tab / date range live in the URL (server-side contract)
  const list = useListParams({ defaultSort: "checkIn:asc", filters: BOOKING_FILTERS });
  const searchTerm = list.filters.q;
  const activeTab = list.filters.status;
  const [date, setDateState] = React.useState(() => ({
    from: fromIso(list.filters.from) || new Date(),
    to: fromIso(list.filters.to) || addMonths(new Date(), 1),
  }));
  ...
  const listQuery = React.useMemo(() => {
    const q = new URLSearchParams(list.apiParams);
    q.set("search", searchTerm);
    q.set("status", activeTab);
    q.set("from", from ?? "");
    q.set("to", to ?? "");
    return q.toString();
  }, [list.apiParams, searchTerm, activeTab, from, to]);
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const response = await fetch(
        `${API_URL}/booking/admin/analytics-filter?${listQuery}`,
        {
          method: "GET",
          headers: {
  ...
  const renderBookingTable = (bookings) => {
    ...
    return (
      <>
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>
                <SortableHeader label="Check-in" sortKey="checkIn" sort={list} onSort={list.toggleSort} />
              </TableHead>
              <TableHead>
                <SortableHeader label="Check-out" sortKey="checkOut" sort={list} onSort={list.toggleSort} />
              </TableHead>
              <TableHead>
                <SortableHeader label="Total" sortKey="price" sort={list} onSort={list.toggleSort} />
              </TableHead>
              <TableHead>
          ...
          >
            {bookings?.map((booking) => (
              <TableRow key={booking._id} data-testid="booking-row">
                <TableCell>
                  <Checkbox
        ...
        {bookings && bookings?.length === 0 && !bookingsPending && (
          <div className="text-center py-10 text-gray-500 font-medium" data-testid="table-empty">
            ...
            {date?.from && date?.to && (
              <>
                No bookings match these filters and dates.
                {list.isDirty ? (
    ...
    <div className="flex-1 space-y-4 px-8 pt-8 pb-24 md:p-8 md:pt-6 bg-gray-200 min-h-screen">
      ...
      <div className="flex-1">
        ...
        <div className="flex flex-col gap-2 md:flex-row md:items-center">
          ...
          <span className="text-sm text-muted-foreground md:ml-auto" data-testid="table-count">
            {bookingsPending ? "" : `${count} ${count === 1 ? "booking" : "bookings"}`}
          </span>
        </div>
      </div>
      <Tabs
      ...
      >
        <TabsListAdmin className="flex flex-col md:flex-row">
          <TabsTriggerAdmin value="all" data-testid="tab-all">All</TabsTriggerAdmin>
          <TabsTriggerAdmin value="confirmed">Confirmed</TabsTriggerAdmin>
          <TabsTriggerAdmin value="pending" data-testid="tab-pending">Pending</TabsTriggerAdmin>
